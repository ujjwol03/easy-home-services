  ...
  try {
    ...
    if (email.includes('@')) {
      ...
      if (account) {
        ...
        // If not found in User, check if it's a Staff member
    ...
    if (role === 'staff' && !account.isApproved) {
      return res.status(403).json({ error: 'Your application is pending admin approval.' });
    }

    // Prepare profile info for token and response
    const nameInfo = role === 'staff' 
      ? { firstName: account.name, lastName: '' } 
